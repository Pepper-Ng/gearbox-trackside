...
- The admin Sessions workspace reads persisted sessions and participants from `ITracksideStore`, with an active live card sourced from `/api/live-session/current`. While the authenticated dashboard's Sessions tab is visible, it performs a lightweight active-only refresh about every 1.5 seconds: live snapshots update immediately, summaries reconcile on live-session transitions, and a short bounded retry waits for the persisted Open Results action. Background refreshes do not reload open result-tab details; manual refreshes and result mutations do. Include/Exclude updates `count_for_history`, per-row Delete removes bad stored sessions, and Delete Empty Sessions clears placeholder rows that have no participants, no completed laps, or no known track.
...
- Tracker colours use a durable name-to-colour history and a fixed, high-contrast palette. At the start of a session, historical colours are claimed by first-seen join order and remaining drivers receive the first free palette colour. Colours are reserved for the full session, so rank changes, brief source gaps, refreshes, and reconnects cannot swap a driver's colour.
