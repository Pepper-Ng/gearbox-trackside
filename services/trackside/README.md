...
- The admin Sessions tab is a stored history browser, not a live-session monitor. It reads persisted sessions and participants from `ITracksideStore`; toggling session inclusion updates `count_for_history`, per-row Delete removes bad stored sessions, and Delete Empty Sessions clears placeholder rows that have no participants or no known track.
